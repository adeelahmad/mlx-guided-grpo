...
class MonitorConfig:
    ...
    kl_mean: ThresholdConfig = field(default_factory=lambda: ThresholdConfig(
        name="KL Divergence",
    ...
    correctness: ThresholdConfig = field(default_factory=lambda: ThresholdConfig(
        name="Correctness",
        good_threshold=0.45,
    ...
    reward: ThresholdConfig = field(default_factory=lambda: ThresholdConfig(
        name="Reward",
        good_threshold=0.50,
...
class TrainingMonitor:
    ...
    def log_step(
        ...
        corr_status = self.config.correctness.check(correctness_value) if correctness_value is not None else Status.UNKNOWN
        rew_status = self.config.reward.check(reward_value) if reward_value is not None else Status.UNKNOWN
        ...
        if self.config.stop_on_critical:
            if kl_value is not None and self._check_stop_condition('kl', kl_status):
                should_stop = True
            if correctness_value is not None and self._check_stop_condition('correctness', corr_status):
                should_stop = True
    ...
    def print_summary(self):
        ...
        print(f"{Colors.BOLD}Total Warnings:{Colors.RESET} {Colors.YELLOW}{self.total_warnings}{Colors.RESET}")
        print(f"{Colors.BOLD}Total Criticals:{Colors.RESET} {Colors.RED}{self.total_criticals}{Colors.RESET}")
...
if __name__ == "__main__":
    ...
    for step in range(1, 21):
        ...
        metrics = {
            ...
            'hierarchical_rewards_mean': 0.35 + random.gauss(0, 0.05) + step * 0.01,  # Improving correctness
