...
def grpo_loss(
    ...
    # CRITICAL: This ensures curriculum scaffolding is ALWAYS masked from loss, regardless of type
    if scaffold_token_counts is not None:
        ...
        for i in range(len(prompt_lengths)):
            ...
            total_masked_tokens += scaffold_count

        # Validation: Log masking for debugging
        if total_masked_tokens > 0:
            import os
            if os.environ.get("DEBUG"):
                from tqdm import tqdm
                tqdm.write(
                    f"  [CURRICULUM MASK] Masked {total_masked_tokens} scaffold tokens from loss "
                    f"across {len([c for c in scaffold_token_counts if c > 0])} completions"
                )
