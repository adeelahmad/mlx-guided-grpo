...
def compute_config_hash(config: Any, tokenizer: Any = None) -> str:
    ...
    config_keys = [
        'prompt_feature', 'answer_feature', 'system_feature', 
        'type_feature', 'train_mode', 'text_feature', 'chat_feature',
        'completion_feature', 'max_seq_length',
        # Cross-sampling config
        'cross_sampling_enabled', 'cross_sampling_ratio',
...
def build_length_index(
    ...
    logger.info(f"Length index: {total} samples in {num_buckets} buckets (avg {avg_per_bucket:.1f}/bucket)")
